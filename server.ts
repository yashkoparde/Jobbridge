...
async function syncFromSupabase(force = false) {
  ...
  try {
    ...
    const timeoutPromise = new Promise<{ error: string } | null>((resolve) => {
      ...
      }, 1500); // 1.5 seconds maximum database checkout time
    });

    const result = await Promise.race([fetchPromise, timeoutPromise]);

    if (result && 'error' in result && result.error === "timeout") {
      console.warn("syncFromSupabase: Connection timed out (1.5s limit reached). Using cached/local high-fidelity schema.");
      return;
    }

    const [uRes, cRes, jRes, aRes, sRes] = result as any[];
...
if (isSupabaseConfigured) {
  try {
    ...
    console.error("JobBridge: Failed to initialize Supabase client safely:", err.message || err);
    supabase = null;
  }
} else {
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (supabase) {
      // Execute inserts in the background without blocking the rapid HTTP response path
      supabase.from('users').insert([newUser]).then(({ error }) => {
        if (error) console.error("Non-blocking insert user error:", error);
      }).catch(err => {
        console.error("Non-blocking insert user crashed:", err);
      });

      if (company) {
        supabase.from('companies').insert([company]).then(({ error }) => {
          if (error) console.error("Non-blocking insert company error:", error);
        }).catch(err => {
          console.error("Non-blocking insert company crashed:", err);
        });
      }
    }
